...
export const ResidenceCard = ({
  ...
  const [newLabel, setNewLabel] = useState(residence.residence);
  ...
  const handleUpdate = () => {
    ...
    if (trimmedLabel === residence.residence) {
      setIsEditing(false);
      return;
    }

    onUpdate({ id: residence.id, newResidence: trimmedLabel });
    setIsEditing(false);
  };
  ...
  const handleCancel = () => {
    setNewLabel(residence.residence);
    setIsEditing(false);
  };
